...
class Connection {
  ...
  void onInitializeRequest(
    Future<dynamic> Function(InitializeParams) handler,
  ) {
    onRequest('initialize', (params) async {
      final initParams = InitializeParams(params.value);
      final result = await handler(initParams);

      return {
        'capabilities': {
          // Указываем, как сервер будет синхронизировать текст
          // 1 = Full (весь файл целиком), 2 = Incremental
          'textDocumentSync': 1,

          // Сообщаем, что мы умеем подсвечивать ошибки/предупреждения
          'diagnosticProvider': {
            'interFileDependencies': false,
            'workspaceDiagnostics': false,
          },

          // Если хотите поддержку "Go to Definition"
          'definitionProvider': true,

          // Если хотите поддержку автодополнения
          'completionProvider': {
  ...
  void onDocumentDiagnosticRequest(
    Future<List<Map<String, dynamic>>> Function(String uri) handler,
  ) {
    onRequest('textDocument/diagnostic', (params) async {
      final uri = params.value['textDocument']['uri'] as String;
      final items = await handler(uri);
  ...
  void onRequest<R>(
    ...
    _peer.registerMethod(method, (params) => handler(params));
  }
